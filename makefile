...
solver: src/solver/Solver.h src/solver/Solver.cpp src/solver/StdRepresentative.h src/solver/StdRepresentative.cpp
...
main: src/main/Main.cpp src/main/Marcio_SandBox.cpp src/main/Fabio_SandBox.cpp tools solver
	...
	g++ -Wextra -ansi -O3 -funroll-all-loops -DIL_STD $(INCCPLEX) $(CONCPLEX) lib/time.o lib/set.o lib/graph.o lib/reader.o src/main/Main.cpp -o bin/main $(LIBCPLEX)	
	g++ -Wextra -ansi -O3 -funroll-all-loops -DIL_STD $(INCCPLEX) $(CONCPLEX) lib/time.o lib/set.o lib/graph.o lib/reader.o src/main/Marcio_SandBox.cpp -o bin/marcio $(LIBCPLEX)
	g++ -Wextra -ansi -O3 -funroll-all-loops -DIL_STD $(INCCPLEX) $(CONCPLEX) lib/time.o lib/set.o lib/graph.o lib/reader.o src/main/Fabio_SandBox.cpp -o bin/fabio $(LIBCPLEX)